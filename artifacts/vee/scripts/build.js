...
// Keep the static build's temporary Metro instance separate from the shared
// mockup preview server, which commonly occupies port 8081 in this workspace.
// The port can still be overridden by the build environment when needed.
...
async function downloadManifest(platform) {
  ...
  try {
    ...
    const candidates = [
      `/manifest?platform=${platform}&dev=false&minify=true`,
      `/_expo/manifest?platform=${platform}&dev=false&minify=true`,
    ];
    let response;
    let lastStatus = 'no response';
    for (const route of candidates) {
      const candidate = await fetch(`http://localhost:${metroPort}${route}`, {
        headers: {
      ...
      lastStatus = `${candidate.status} at ${route}`;
    }
    if (!response) {
